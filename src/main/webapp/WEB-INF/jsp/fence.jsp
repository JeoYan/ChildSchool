...
<script type="text/javascript">
	...
	function initLine(){
		...
		var driving1 = new BMap.DrivingRoute(map);  // 驾车实例,并设置回调
		driving1.search(spoi1, epoi1);                                       // 搜索一条线路
		var driving2 = new BMap.DrivingRoute(map);  // 驾车实例,并设置回调
		driving2.search(spoi2, epoi2);                                       // 搜索一条线路
		var driving3 = new BMap.DrivingRoute(map);  // 驾车实例,并设置回调
		driving3.search(spoi3, epoi3);                                         // 搜索一条线路
		var driving4 = new BMap.DrivingRoute(map);  // 驾车实例,并设置回调
		driving4.search(spoi4, epoi4);
		drivings.push(driving1,driving2,driving3,driving4);




		for (var i = 0; i <drivings.length ; i++) {
			run(drivings[i],labels[i],i);
		}
	...
	function run(item,babyLabel,m){



		item.setSearchCompleteCallback(function(){
			var pts = item.getResults().getPlan(0).getRoute(0).getPath();    //通过驾车实例，获得一系列点的数组
			var paths = pts.length;    //获得有几个点
			var flag=true;
			var carMk = new BMap.Marker(pts[0],{icon:myIcon});
			var label = new BMap.Label(babyLabel,{offset:new BMap.Size(20,-10)});
			carMk.setLabel(label);
			carMk.setTitle(m+2);
			map.addOverlay(carMk);
			i=0;
