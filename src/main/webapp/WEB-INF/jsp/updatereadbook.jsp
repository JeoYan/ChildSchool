...
<div id="LAY-user-login" style="display: none;">
	...
	<div class="layui-card-body">
		<div style="text-align: center;">
			...
			<div class="layui-inline">
				<button class="layui-btn layuiadmin-btn-useradmin" data-type="add">确认修改</button>
			</div>
		</div>
	</div>
	<div style="text-align: center">
...
<script>
	...
	layui.use('table', function () {
		...
		var bName=$("#bName").val();
		// alert($("#bName").val());
		// alert($("#hideBookid").val());

		//第一个实例
		table.render({
			elem: '#demo'
			, url: '${pageContext.request.contextPath}/readBook/getBookPage.action?bName='+bName //数据接口
			, page: true //开启分页
			, limit: 5
			, done: function () {
				...
				, add: function () {
					...
					if (bName != null && bName != "") {
						...
						if(confirm("确认修改？")){
							...
							$.ajax({
								type: "POST",//提奥的方式
								url: "/ChildSchool/readBook/updateBookName.action",//提交的地址
								data: ob,//提交的数据
								dataType: "json",//希望返回的数据类型
								async: true,//异步操作
								success: function (num) {//成功的方法  msg为返回数据
