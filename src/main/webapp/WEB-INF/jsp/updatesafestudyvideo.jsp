...
<form class="layui-form" action="" lay-filter="example" style="margin-top: 30px;text-align: center">
	...
	<div class="layui-form-item">
		...
		<div class="layui-input-inline">
			<input type="text" name="username" id="safeName"  placeholder="请输入视频名称" lay-verify="title" autocomplete="off"
			       class="layui-input">
			<input type="hidden" name="username" id="safeId"  lay-verify="title" autocomplete="off"
			       class="layui-input">
			<input type="hidden" name="username" id="oldSafeName"  lay-verify="title" autocomplete="off"
			       class="layui-input">
		</div>
	</div>
	<div class="layui-form-item" style="margin:0 auto;text-align: center">
		...
		<div class="layui-input-inline">
			<input type="text" class="layui-input"  id="rangeDate" placeholder="yyyy-MM-dd">
		</div>
	</div>
	<div class="demoTable">
...
<script>
	...
	layui.use('upload', function () {
		...
		upload.render({
			elem: '#test8'
			, url: '/ChildSchool/BackAction/updateVideo.action'
			, accept: 'video'
			,exts: 'mp4' //只允许上传压缩文件
			...
			,before: function(obj){
				...
				this.data={"safeName": $('#safeName').val(), "rangeDate":$('#rangeDate').val(),"oldSafeName": $('#oldSafeName').val(), "safeId":$('#safeId').val()};
