...
public class SystemLogAspect
{
	...
	@Resource
	private TjzBackService backService;
	@Resource
	private HttpServletRequest req;

	@Pointcut("execution (* com.great.childschool.controller..*.back*(..))")
		...
		try
		{
			...
			for (Method method : methods)
			{
				if (method.getName().equals(methodName) && method.getParameterTypes().length == params.length)
				{
					...
					TbLog log=new TbLog();
					log.setOperationalMatters(operationName);
					log.setOperator(req.getSession().getAttribute("adminName").toString());
					Date day=new Date();
					SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
					log.setOperationTime(df.format(day));
					int flag = backService.addLog(log);
					if (flag > 0)
					{
